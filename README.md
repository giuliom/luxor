...
| `PUBLIC_BASE_URL` | Derived | Absolute public origin for canonical and Open Graph URLs, hreflang alternates, the sitemap, and `robots.txt`; development derives the listener address, production the first CORS origin |
...
The console's WebAssembly card benchmarks a prime sieve compiled from Rust ([`wasm/`](wasm/)) against the identical sieve in JavaScript, cross-checking that both counts agree. After one untimed warmup, each displayed timing is the average of 10 measured iterations. The module is plain `wasm32-unknown-unknown` output with a C-ABI export — no bindings generator or JS glue — and the page loads it with standard `WebAssembly.instantiateStreaming`, which requires the `application/wasm` content type the module is served with. The page hands the script the module's content-addressed URL (`/assets/demo.<fingerprint>.wasm`), so browsers cache it indefinitely; `/demo.wasm` remains as a revalidated alias. The site's Content-Security-Policy allows this with the CSP3 `'wasm-unsafe-eval'` keyword, which permits WebAssembly compilation while continuing to forbid JavaScript `eval`.
...
Dictionary entries are stable semantic keys (`errors.code.not_found`, never the English sentence) holding whole messages with named `{placeholder}` slots — sentences are never concatenated from fragments, so word order is free to differ per language. Plural forms live under CLDR category keys (`telemetry.spans.one` / `.other`) selected client-side with `Intl.PluralRules`, and dates, numbers, and unit-tagged durations go through `Intl.DateTimeFormat`/`Intl.NumberFormat` rather than manual formatting. The page's dictionary is inlined as a non-executing JSON data block (allowed by the CSP, which continues to forbid inline scripts; translated values are HTML-escaped when rendered, and `</` is escaped in the block, so a translation can never inject markup or terminate its element), which keeps client-side strings to one language and zero extra requests. The API stays language-neutral: it returns stable error codes and role wire names (`admin`, `user`), and the console translates them — role names wherever they appear, error codes with the server's request-specific detail visibly appended rather than passed off as translated.

Each rendered page carries its `<html lang>` and `dir` (and is served with a matching `Content-Language` header), a translated title and meta description, a self-referencing canonical URL, and reciprocal `hreflang` alternates (plus `x-default`); `/sitemap.xml` lists every language version, and the language selector is ordinary crawlable links, so search engines discover translations without executing JavaScript or guessing from headers. The absolute origin in those URLs comes from `PUBLIC_BASE_URL` or its documented derivation. Trailing-slash variants (`/en/`) redirect permanently to the canonical form, and language-prefixed URLs make locale part of any CDN cache key by construction.

Unit tests in [`src/i18n.rs`](src/i18n.rs) gate CI on translation health: dictionaries must parse, carry identical key sets (missing and obsolete keys both fail), keep the same placeholders per key across languages, contain no markup, define every key the template, script, and server-side renderer look up, and define no key nothing uses; rendered pages must contain no unresolved placeholder and correct canonical/hreflang metadata. A client-side lookup that still misses renders its key on screen — a loud, diagnosable failure rather than a silent English fallback. To add a language: add a `Locale` variant (the compiler then asks for its tag, path, endonym, and dictionary file), list it in `SUPPORTED_LOCALES`, and write `locales/<lang>/common.json`. Routes, hreflang alternates, the language selector, and the sitemap are all generated from that list, so the template never changes, and the tests enforce completeness. Translated slugs beyond the language prefix are unnecessary while the console is a single page.

## Server-side rendering and delivery

The console pages are rendered on the server once, at startup, rather than per request or in the browser: everything a page shows that is fixed for the life of the process is in its first response. That covers the translated text and metadata described above, the permissions card's matrix (drawn from the same fixed grants the endpoints enforce), and the service card's runtime badge. A page therefore needs no request to finish rendering, and crawlers and visitors without JavaScript see the complete content. The one call the page makes at startup is the refresh that restores a session, which cannot be server-rendered because the server never knows who is signed in when it serves a page (see above).

Every non-API response — the pages, `/sitemap.xml`, `/robots.txt`, and the static files — is a representation prebuilt at startup ([`src/assets.rs`](src/assets.rs)): it carries a strong `ETag`, and a gzip copy is served to clients that send `Accept-Encoding: gzip`, with `Vary: accept-encoding`. The request path does no templating, hashing, or compression. The URL decides the cache policy:

| URLs | `Cache-Control` | Why |
| --- | --- | --- |
| `/assets/<name>.<fingerprint>.<ext>` | `public, max-age=31536000, immutable` | Content-addressed: the fingerprint is a SHA-256 prefix of the file, so new content means a new URL, and the rendered pages reference only this form. An outdated fingerprint answers `404` rather than serving current bytes under an immutable old URL |
| Pages, `/sitemap.xml`, `/robots.txt`, and the stable names `/styles.css`, `/script.js`, `/favicon.svg`, `/demo.wasm` | `no-cache` | The content changes with a deployment, so every use revalidates, and an unchanged representation answers `304 Not Modified` without a body |

A repeat visit costs one conditional request for the page and none for its assets. During a rolling deployment, a page rendered by one version can request an asset fingerprint from an instance running the other; that request gets a `404`, and reloading the page fixes it.

Compression is limited to these static representations. API responses are never compressed, because they carry access tokens alongside request-influenced content, which is the precondition for BREACH-style length attacks.

`/robots.txt` names the sitemap and disallows `/api/`. The API's JSON is not content, and crawler traffic would spend the per-IP rate-limit budgets. Pages also carry Open Graph title, description, and URL metadata for link previews.
